...
Record = namedtuple(
    "Record",
    [
        "year", "wmt", "testset", "langpair", "src_lang", "tgt_lang",
        "system", "segment_id", "doc_id", "source", "reference",
        "hypothesis", "human_score", "human_score_level",
    ],
)
...
# 2008-2011 and "cs" from 2012).  Normalise for the ``src_lang`` / ``tgt_lang``
# fields while keeping the on-disk ``langpair`` string untouched.
...
def _summarise(data_root=DEFAULT_DATA_ROOT):
    ...
    for r in iter_records(data_root):
        total += 1
        per_year[r.wmt] += 1
        langpairs.add((r.wmt, r.langpair))
        systems.add((r.wmt, r.langpair, r.system))
